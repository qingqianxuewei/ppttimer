...
global pt_IniFile := A_ScriptDir "\ppttimer2.ini"
...
global opacity, fontface, fontweight, fontsize, indicator_fontsize := 12, textColor, AheadColor, timeoutColor, backgroundColor, bannerWidth, bannerHeight, bannerPosition, bannerMargin
global stopResetsTimer,  pt_Durations := [], pt_Ahead, pt_PlayFinishSound, pt_FinishSoundFile, pt_PlayWarningSound, pt_WarningSoundFile, sendOnTimeout
global currentIndicator := ""
global currentDurationIndex := 1
...
SwitchDuration:
currentDurationIndex := Mod(currentDurationIndex, pt_Durations.Length()) + 1
; TrayTip, 当前持续时间, % pt_Durations[currentDurationIndex] "秒", 1
IniWrite, %currentDurationIndex%, %pt_IniFile%, status, currentDurationIndex
resetTimer()
return
...
resetTimer() {
  ...
  Loop, %MonitorCount% {
    ...
    GuiControl,, %hText%, % FormatSeconds(pt_Durations[currentDurationIndex])
  }
...
CountDownTimer(){
  ...
  if (remaining != pt_Durations[currentDurationIndex] - elapsed) {
    remaining := pt_Durations[currentDurationIndex] - elapsed
    updateCountDownText()
  }
  if (remaining = 0) {
    sendTimeoutKeys()
...
loadSettings(){
  ...
  InIRead, switchDurationKey, %pt_IniFile%, shortcuts, switchDurationKey, ^#T
  InIRead, currentDurationIndex, %pt_IniFile%, status, currentDurationIndex, 1
  ...
  hotkey, %switchDurationKey%, SwitchDuration
...
loadProfile(idx) {
  ...
  if (idx > 0) {
    ...
    InIRead, ptDurationsStr, %pt_IniFile%, %ProfileSectionName%, Durations, % ""
    if (ptDurationsStr != "") {
        pt_Durations := []
        Loop, Parse, ptDurationsStr, `,
            pt_Durations.Push(A_LoopField)
    }
...
loadDefaultProfile(){
  ...
  InIRead, ptDurationsStr, %pt_IniFile%, Main, Durations, % ""
    if (ptDurationsStr != "") {
        pt_Durations := []
        Loop, Parse, ptDurationsStr, `,
            pt_Durations.Push(A_LoopField)
